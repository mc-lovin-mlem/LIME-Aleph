...
from .own_segmentation import quadratic
...
class SegmentationAlgorithm(BaseWrapper):
    ...
    def __init__(self, algo_type, **target_params):
        ...
        if (self.algo_type == 'quickshift'):
            ...
            self.set_params(**kwargs)
        # custom algorithm for quadratic superpixels
        elif (self.algo_type == 'quadratic'):
            BaseWrapper.__init__(self, quadratic, **target_params)
            kwargs = self.filter_params(quadratic)
            self.set_params(**kwargs)
